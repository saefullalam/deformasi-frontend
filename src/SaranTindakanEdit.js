...
const SaranTindakanEdit = () => {
  ...
  const getData = (startDateParam,endDateParam) => {
    axios.post(`http://127.0.0.1:5000/select_sarantindakan/${startDateParam}/${endDateParam}`)
      .then(res => {
        setData(res.data);
      })
  }
  ...
  const deleteData = (rec,start_date,end_date) => {
    ...
    const data = {
      ID       : rec.id,
      Tanggal  : rec.tanggal,
      Sarantindakan    : rec.sarantindakan
    }

    axios({
      method: "POST",
      url:"http://127.0.0.1:5000/delete_sarantindakan",
      data: data
    })
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'sarantindakan',
      key: 'sarantindakan',
      ...getColumnSearchProps('sarantindakan'),
...
export default SaranTindakanEdit;