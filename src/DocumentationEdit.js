...
const DocumentationEdit = () => {
  ...
  const getData = (startDateParam,endDateParam) => {
    axios.post(`http://127.0.0.1:5000/select_documentation/${startDateParam}/select`)
      .then(res => {
        setData(res.data);
  ...
  const deleteData = (rec,start_date,end_date) => {
    ...
    axios({
      method: "POST",
      url:"http://127.0.0.1:5000/delete_documentation",
      data: data
    })
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'image',
      render: (_, record) => (
        <Image width={100} src={'http://127.0.0.1:5000/select_documentation/'+ record.id + '/render'} />
      ),
    },
    {
      render: (_, record) => (
